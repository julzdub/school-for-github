...
public class Trie2 {
	...
	public LinkedList wordsPrefixedBy(String s) { // returns a LinkedList of all words with the prefix s
		return wordsPrefixedBy(this.root, s);
	}

	private LinkedList wordsPrefixedBy(TrieNode node, String s) {
		char firstLetter = s.charAt(0);
		TrieNode child = node.children.get(firstLetter);
		if(child != null) {
			if(s.charAt(1) != null) {
				wordsPrefixedBy(child, s.substring(1));
			}
